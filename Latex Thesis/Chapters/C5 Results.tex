...
\begin{figure}[h]
    ...
    \caption{a general network where i and j are connected through multiple paths}
...
\begin{figure}[h]
    ...
    \caption{illustration of the in-neighbours}
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
In particular, from \autoref{eq:bound on variance for network variable from the sum of ingoing weights} and \autoref{fig:2 chain MI error} and \autoref{fig:3 and 4 chain MI error}, we suspect that the bleeding effects observed for the Gaussian chain won't appear in this case.
\begin{figure}[h]
    \centering
