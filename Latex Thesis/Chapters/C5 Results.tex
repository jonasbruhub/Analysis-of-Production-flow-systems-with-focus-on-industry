...
\end{figure}
% Let $\mathcal{G}$ be the graph that represents the network and label the nodes such that they form a topological ordering

Then the resulting $G_{dir}$ of the weights is clearly upper triangular a directed edge can not go from an index $j$ to another index $i$ such that $i<j$. Then, for each node $i$, the predecessors is a subset of the nodes with $j < i$ and furthermore, the set immediate predecessors (or the \textit{in-neighbors}) which we shall denote $N^-_i$ is a subset of these
