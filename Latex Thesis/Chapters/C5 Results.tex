...
Using \autoref{prop:MI bivariate gaussian}, we can transform $G_{obs}$ from the previous section to be in terms of mutual information instead of correlation. It is not hard to show that in this case, even for Gaussian chains, there is some error as we shall see shortly.

% Figure om fejl man laver step by step. Ud fra figur er det også mest 2-, 4-, etc. led ude der er fejl på
...
\begin{figure}[H]
    ...
    \begin{subfigure}[t]{0.49\textwidth}
        ...
        % \label{fig:Gaussian 3x3 large s}
    \end{subfigure}
    \caption{Triangular, mutual information, cutoff $2.1 \, 10^{-2}$. The rest of the figures in appendix (other cutoff). Other cutoff can be seen in \autoref{fig:Gaussian chain symmetric G_obs using mutual information different cutoff}}
    % \label{}
\end{figure}
...
\begin{figure}[h]
    ...
    \begin{subfigure}[t]{0.49\textwidth}
        ...
        % \label{fig:Gaussian 3x3 large s}
    \end{subfigure}
    \caption{symmetric, mutual information, cutoff $4.88 \cdot 10^{-2}$. Performance almost on par with correlation.}
    % \label{}
\end{figure}
