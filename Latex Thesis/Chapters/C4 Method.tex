...
Reflection (ikke så god)

\subsection{Boundary corrected KDE}
Den fra artikel i kina

We shall also try the method briefly presented in \cite{Nonparametric-copula-entropy-and-network-deconvolution-method-for-causal-discovery-in-complex-manufacturing-systems}, which is based on \cite{Jones1993SimpleBC}. It is a first order kernel but only specified in one dimension and initially only for a lower bound. The 

Furthermore, in \cite{A-simple-nonnegative-boundary-correction-method-for-kernel-density-estimation}, Jones and Foster propose an extension to handle the possible negativity of the estimate density.
...
using the one from china, we get that for large correlation, the algorithm on 500x500 does not do well (refer to image on convergence when accurate PDF -> improved scheme by adaptively increasing the mesh size if one gets a large MI)


% R documentation for bckden
https://search.r-project.org/CRAN/refmans/evmix/html/bckden.html

% suicide data for at sammenligne implementering

Hvad med denne: 
\cite{Estimating-mutual-information-Kraskov}



% @article{0833f3a6-edb6-3c71-b4f9-5491bc3f60ad,
%  ISSN = {00359238},
